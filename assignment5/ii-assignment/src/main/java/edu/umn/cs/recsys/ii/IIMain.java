...
public class IIMain {
	...
	private static Map<Long, List<Recommendation>> recommendations;
	private static List<Recommendation> globalRecommendations;

	static {
		recommendations = new HashMap<Long, List<Recommendation>>();
	}
	...
	public static void main(String[] args) {
		...
		if (basket != null) {
			...
			globalRecommendations = new ArrayList<Recommendation>();
		...
		for (Map.Entry<Long, Set<Long>> scoreRequest : toScore.entrySet()) {
			...
			for (long item : items) {
				...
				if (!recommendations.containsValue(user)) {
					userRecommendations = new ArrayList<Recommendation>();
				} else {
					userRecommendations = recommendations.get(user);
				}
	...
	private static LenskitConfiguration configureRecommender() {
		...
		config.bind(ItemScorer.class).to(SimpleItemItemScorer.class);
	...
	public static List<Recommendation> getGlobalRecommendationsFor() {
		return (globalRecommendations != null) ? globalRecommendations : new ArrayList<Recommendation>();
	}
